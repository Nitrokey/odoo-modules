...
class FirebaseNotifyController(http.Controller):
    ...
    def send_test_notification(self):
        ...
        try:
            ...
            # Check if this is a test token (auto-generated by system)
            if user.firebase_token.startswith('test-token-'):
                return {
                    'success': False,
                    'error': 'Cannot send notifications to test token. Please click "Enable Browser Notifications" first to get a real Firebase token.'
                }
            ...
            _logger.info(f"Firebase configuration found - Web Project ID: {firebase_config.web_project_id}")
            
            # Get service account project ID for comparison
            credentials_dict = firebase_config.get_firebase_credentials()
            if credentials_dict:
                service_project_id = credentials_dict.get('project_id', 'Unknown')
                _logger.info(f"Service account Project ID: {service_project_id}")
                
                if firebase_config.web_project_id != service_project_id:
                    return {
                        'success': False,
                        'error': f'Project mismatch! Web app project ({firebase_config.web_project_id}) differs from service account project ({service_project_id}). Please ensure both are from the same Firebase project.'
                    }
            else:
                _logger.error("Could not get service account credentials")
