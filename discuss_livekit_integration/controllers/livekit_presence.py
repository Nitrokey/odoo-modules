...
class LivekitPresenceController(http.Controller):
    @http.route("/mail/livekit/channel/join_call", methods=["POST"], type="json", auth="public")
    ...
    def join_call(self, channel_id, camera=False):
        ...
        if not session_sudo:
            ...
            # Mimic base RTC semantics: when the first participant starts a call in a non-channel
            # thread, ring other connected members.
            try:
                session_count = (
                    request.env["discuss.channel.livekit.session"]
                    .sudo()
            ...
            # If the session already exists (e.g., tab reload), broadcast an UPDATE so other
            # participants still get an immediate presence signal.
            session_sudo._update_and_broadcast({"is_camera_on": camera})

        # Return minimal presence info; frontend will still obtain LiveKit token via /livekit/token
        sessions = request.env["discuss.channel.livekit.session"].sudo().search([("channel_id", "=", channel.id)])
        return {
    ...
    @http.route("/mail/livekit/channel/leave_call", methods=["POST"], type="json", auth="public")
    @add_guest_to_context
    ...
    def update_and_broadcast(self, session_id, values):
        ...
        if not isinstance(values, dict):
            return
        session_sudo = request.env["discuss.channel.livekit.session"].sudo().browse(session_id).exists()
        if not session_sudo:
            return
    ...
    @http.route("/discuss/livekit/channel/ping", methods=["POST"], type="json", auth="public")
    @add_guest_to_context
    def ping(self, channel_id, livekit_session_id=None, check_session_ids=None):
        ...
        if livekit_session_id:
            ...
            session = (
                request.env["discuss.channel.livekit.session"].sudo().browse(livekit_session_id).exists()
                if livekit_session_id
        ...
        current_sessions = (
            request.env["discuss.channel.livekit.session"].sudo().search([("channel_id", "=", channel_id)])
        )
