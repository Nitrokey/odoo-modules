...
class LivekitController(http.Controller):
    ...
    def livekit_token(self, channel_id):
        ...
        channel = request.env["discuss.channel"].search([("id", "=", channel_id)], limit=1)
        if not channel:
            raise NotFound()
        ...
        livekit_enabled = str2bool(raw_enabled if isinstance(raw_enabled, str) else "False")
        raw_livekit_url = icp.get_param("discuss_livekit_integration.livekit_server_url")
