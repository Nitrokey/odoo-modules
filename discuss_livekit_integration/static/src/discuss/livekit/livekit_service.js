...
import {HOST_TAKEOVER_DELAY_MS, LIVEKIT_HOST_MSG, parseLivekitIdentity} from "./livekit_utils";
...
export const livekitService = {
    ...
    start(env) {
        ...
        function getRtcSessionForLivekitIdentity(channel, identity) {
            ...
            return sessions.find((s) => (partnerId && s.partnerId === partnerId) || (guestId && s.guestId === guestId));
        }
            ...
            return (nextState) => {
                ...
                if (stateStr === "reconnecting") {
                    ...
                    if (!reconnectToastShown) {
                        ...
                        try {
                            env.services.notification.add(_t("Connection lost. Reconnecting..."), {
                                type: "warning",
                                sticky: true,
                            });
                        } catch (e) {
        ...
        const screenShareManager = new LivekitScreenShareManager(env, state, warn, publicationToType, getLivekitClient);
                ...
                onChange(store.settings, "audioInputDeviceId", async () => {
                    ...
                    try {
                        ...
                        if (state.channel && state.isHost) {
                            await presence?.updatePresence(state.channel, {is_muted: true});
                        }
                    }
                });
            }
        ...
        async function leave() {
            ...
            try {
                ...
                try {
                    // eslint-disable-next-line no-unused-expressions
                    env.services["discuss.livekit_rtc_adapter"]?.updateTalkingStates?.(channel?.id, []);
                } catch (e) {
        ...
        async function setMicEnabled(enabled) {
            ...
            try {
                ...
                env.services.notification.add(`Microphone failed: ${e?.message || e}`, {type: "warning"});
                try {
                    await microphoneManager.setMicrophoneEnabled(false);
                } catch {
                    // Ignore
            ...
            if (state.channel && state.isHost) {
                await presence?.updatePresence(state.channel, {is_muted: !state.micEnabled});
            }
        }
        ...
        async function join(channel, {audio = true, camera = false} = {}) {
            ...
            try {
                ...
                const selfSessionId = presence?.state?.selfSessionIdByChannelId?.get(channel.id);
                ...
                env.services.notification.add(`LiveKit call failed: ${error?.message || error}`, {type: "danger"});
                await leave();
                return false;
            } finally {
                state.connecting = false;
        ...
        async function setDeaf(enabled) {
            ...
            // Only host should update presence
            if (state.channel && state.isHost) {
                await presence?.updatePresence(state.channel, {is_deaf: state.deafened});
            }
        }
        ...
        function isInCall(channel) {
            return Boolean(state.connected && state.channel && state.channel.eq(channel));
        }
