...
import {ensureTrackProcessorsLoaded, getLivekitClient, getLivekitTrackProcessors} from "./livekit_sdk_loader";
...
export class LivekitCameraManager {
    ...
    _getTrackProcessor(track) {
        try {
            return typeof track?.getProcessor === "function" ? track.getProcessor() : null;
        } catch {
            return null;
        }
    }
    ...
    _removeLocalCameraBridge(ctx) {
        ...
        if (selfRtcSessionId) {
            this._callIfFn(adapter, "removeLivekitVideoTrackForRtcSessionId", selfRtcSessionId, {type: "camera"});
            this._callIfFn(adapter, "removeStreamForRtcSessionId", selfRtcSessionId, {type: "camera"});
        }
        this._callIfFn(adapter, "removeLivekitVideoTrackForIdentity", channelId, identity, {type: "camera"});
        this._callIfFn(adapter, "removeStreamForIdentity", channelId, identity, {type: "camera"});
    }

    _setLocalCameraBridge(ctx, track) {
        const {adapter, channelId, identity, selfRtcSessionId} = ctx;
        if (selfRtcSessionId) {
            this._callIfFn(adapter, "setLivekitVideoTrackForRtcSessionId", selfRtcSessionId, {
                type: "camera",
                track,
            });
        }
        this._callIfFn(adapter, "setLivekitVideoTrackForIdentity", channelId, identity, {type: "camera", track});
    ...
    _setupBlurWatchers() {
        ...
        try {
            ...
            onChange(store.settings, ["edgeBlurAmount", "backgroundBlurAmount"], async () => {
                await this.withCameraLock(async () => {
    ...
    async _applyBlurFromSettings({forceRecreate = false} = {}) {
        if (!this.state.connected || !this.state.cameraEnabled || !this.localPublishedCameraTrack) {
            return;
        }
    ...
    async _createBackgroundBlurProcessor() {
        ...
        const supported =
            (typeof tp.supportsBackgroundProcessors === "function" && tp.supportsBackgroundProcessors()) ||
            (typeof tp.supportsModernBackgroundProcessors === "function" && tp.supportsModernBackgroundProcessors());
        if (!supported) {
    ...
    async _createLocalCameraTrack() {
        ...
        try {
            ...
            if (mediaStreamTrack && typeof mediaStreamTrack.addEventListener === "function") {
                mediaStreamTrack.addEventListener("ended", () => {
                    this.withCameraLock(async () => {
                        ...
                        if (this.state.channel && this.state.isHost && typeof presence?.updatePresence === "function") {
                            await presence.updatePresence(this.state.channel, {is_camera_on: false});
                        }
        ...
        return this.withCameraLock(async () => {
            ...
            if (shouldEnable) {
                try {
                    ...
                    const LivekitClient = getLivekitClient();
                    const cameraSource = LivekitClient?.Track?.Source?.Camera || LivekitClient?.TrackSource?.Camera;
                    ...
                    this.env.services.notification.add(`Camera failed: ${e?.message || e}`, {
                        type: "warning",
                    });
            ...
            if (this.state.channel && this.state.isHost) {
                if (typeof presence?.updatePresence === "function") {
                    await presence.updatePresence(this.state.channel, {is_camera_on: this.state.cameraEnabled});
                }
            }
