...
patch(CallParticipantVideo.prototype, {
    ...
    _update() {
        ...
        const isSynthetic = Boolean(rtcSession && typeof rtcSession.id === "number" && rtcSession.id < 0);
        const livekitTrack = livekitConnected && isSynthetic ? this._livekitRtcAdapter?.getLivekitVideoTrack?.(rtcSession.id, type) : null;

        // Detach previous LiveKit track if it no longer matches.
        if (this.__livekitAttachedTrack && this.__livekitAttachedTrack !== livekitTrack) {
            try {
