...
function getEffectiveCameraFromOngoingSessions({presence, channel, camera}) {
    ...
    const ongoingHasVideo = sessions.some((s) => Boolean(s?.isCameraOn || s?.isScreenSharingOn));
...
function enterNativeCallSafely({adapter, presence, channel}) {
    try {
        ...
        if (adapter && typeof adapter.enterCall === "function") {
            adapter.enterCall(channel.id, presence?.state?.selfSessionIdByChannelId?.get(channel.id));
        }
...
patch(Rtc.prototype, {
    async toggleCall(channel, {audio = true, fullscreen, camera} = {}) {
        ...
        const effectiveCamera = getEffectiveCameraFromOngoingSessions({presence, channel, camera});
