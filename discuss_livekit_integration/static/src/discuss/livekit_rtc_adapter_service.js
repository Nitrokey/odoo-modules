    ...
    async start(env, services) {
        ...
        function getOrCreateChannelMember(presenceSession) {
            ...
            const personaId = Number(
                presenceSession.partnerId || presenceSession.guestId
            );
            if (!personaId) {
                return undefined;
            }
        ...
        function notifyRaiseHandTransition({
            presenceSession,
            rtcSessionId,
            wasRaisingHand,
            isRaisingHand,
            ...
            try {
                ...
                const selfId = presence?.state?.selfSessionIdByChannelId?.get(
                    presenceSession.channelId
                );
                const isSelf =
                    Boolean(selfId) && Number(presenceSession.id) === Number(selfId);
                ...
                if (rtcSession.id < 0 && !wantedRtcIds.has(rtcSession.id)) {
                    try {
                        if (rtcSession.raisingHand) {
                            env.services["discuss.rtc"]?.removeCallNotification?.(
                                "raise_hand_" + rtcSession.id
                            );
                        }
                    } catch (e) {
        ...
        function setStreamForIdentity(
            channelId,
            identity,
            {type = "camera", mediaStreamTrack} = {}
        ) {
            if (!mediaStreamTrack) {
                return;
            }
        ...
        function setLivekitVideoTrackForIdentity(
            channelId,
            identity,
            {type = "camera", track} = {}
        ) {
        ...
        function removeLivekitVideoTrackForIdentity(
            channelId,
            identity,
            {type = "camera"} = {}
        ) {
        ...
        function setLivekitVideoTrackForRtcSessionId(
            rtcSessionId,
            {type = "camera", track} = {}
        ) {
        ...
        function removeLivekitVideoTrackForRtcSessionId(
            rtcSessionId,
            {type = "camera"} = {}
        ) {
            if (!rtcSessionId) {
        ...
        function setStreamForRtcSessionId(
            rtcSessionId,
            {type = "camera", mediaStreamTrack} = {}
        ) {
        ...
        function updateTalkingStates(channelId, identities = []) {
            ...
            for (const s of sessions) {
                ...
                const identity = s.partnerId
                    ? `partner_${s.partnerId}`
                    : s.guestId
                      ? `guest_${s.guestId}`
                      : undefined;
        ...
        bus.subscribe("discuss.channel.livekit.call/invitation", (payload) => {
            ...
            try {
                ...
                const selfLivekitSessionId =
                    presence?.state?.selfSessionIdByChannelId?.get(channelId);
                if (
                    selfLivekitSessionId &&
                    Number(selfLivekitSessionId) === Number(inviterSessionId)
                ) {
                    return;
                }
                ...
                const inviterRtcSession = store.RtcSession.get(
                    toRtcSessionId(inviterSessionId)
                );
...
registry
    .category("services")
    .add("discuss.livekit_rtc_adapter", livekitRtcAdapterService);
