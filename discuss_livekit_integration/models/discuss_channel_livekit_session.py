...
class DiscussChannelLivekitSession(models.Model):
    ...
    channel_member_id = fields.Many2one("discuss.channel.member", required=True, ondelete="cascade", index=True)
    ...
    def _broadcast(self, action, sessions, *, only_id=False):
        ...
        for channel, channel_sessions in by_channel.items():
            # Use the channel "members" subchannel so that all members receive the notification
            channel._bus_send(
                "discuss.channel.livekit.session/update",
                {
                    "channelId": channel.id,
                    "action": action,
                    "sessions": [s._to_payload(only_id=only_id) for s in channel_sessions],
                },
                subchannel="members",
            )
