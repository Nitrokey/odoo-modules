...
class ProductConfigurator(ProductConfiguratorController):
    ...
    @http.route(['/sale_product_configurator/show_advanced_configurator_website'], type='json', auth="public",
    ...
    def show_advanced_configurator_website(self, product_id, variant_values, **kw):
        ...
        res = super(ProductConfigurator, self).show_advanced_configurator_website(product_id, variant_values, **kw)
        if not res and product.mandatory_product_ids:
            kw.pop('pricelist_id')
            pricelist = request.website.get_current_pricelist()
            return self.show_advanced_configurator(product_id, variant_values, pricelist, **kw)
        return res

    def _show_advanced_configurator(self, product_id, variant_values, pricelist, handle_stock, **kw):
        product = request.env['product.product'].browse(product_id)
        if product.mandatory_product_ids and 'kwargs' in kw and not kw['kwargs']['context'].get('display_optional'):
            product = request.env['product.product'].browse(int(product_id))
            ...
            return request.env['ir.ui.view']._render_template("product_mandatory_products.mandatory_products_modal", {
                'product': product,
        ...
        return super(ProductConfigurator, self)._show_advanced_configurator(product_id, variant_values, pricelist,
                                                                            handle_stock, **kw)
...
class WebsiteSaleExtend(WebsiteSale):
    ...
    def cart_options_update_json(self, product_and_options, goto_shop=None, lang=None, **kwargs):
        if goto_shop == 'true':
            if self.KW:
                ...
                product_and_options = self.update_product_and_options(product_and_options)
            res = super(WebsiteSaleExtend, self).cart_options_update_json(product_and_options, goto_shop=goto_shop,
                                                                          lang=lang, **kwargs)
            self.KW = []
            return res
        else:
    ...
    def check_mendatory_product(self, *args, **product_data):
        ...
        if product_data.get('root_product'):
            root_product_id = request.env['product.product'].sudo().browse(int(product_data['root_product']))
            if root_product_id.mandatory_product_ids:
                if 'products' not in product_data:
                    return {'has_mandatory': False}
                product_in_cart = [i for i in root_product_id.mandatory_product_ids.product_variant_ids.ids if
                                   i in product_data['products']]
                if product_in_cart:
                    return {'has_mandatory': True,
                            'display_optional': True if root_product_id.optional_product_ids else False}
                else:
