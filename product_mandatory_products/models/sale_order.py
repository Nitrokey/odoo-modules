...
class SaleOrder(models.Model):
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if kwargs.get('linked_line_id'):
            ...
            if linked_line.product_id.mandatory_product_ids and order_line.browse(
                    values.get('line_id')).product_id.id in linked_line.product_id.mandatory_product_ids.mapped(
                ...
                if self._context.get('lang', '') == 'de_DE':
                    order_line.browse(values.get('line_id')).write({
                        'name': order_line.browse(values.get('line_id')).name.replace("Option für:",
                                                                                      "Verpflichtend für:"),
                    })
                else:
                    order_line.browse(values.get('line_id')).write({
