                ...
                for po_l in stock_id.purchase_id.order_line:
                    if po_l.product_id == product:
                        ...
                        if po_l.product_id.product_tmpl_id.uom_po_id != po_l.product_uom:
                            ...
                            price = po_l.product_uom._compute_bom_price(bom, boms_to_recompute=boms_to_recompute)
                        product.standard_price = price
            elif product.bom_count > 0:
                    product.standard_price = product._compute_bom_price(bom, boms_to_recompute=boms_to_recompute, byproduct_bom=True)
