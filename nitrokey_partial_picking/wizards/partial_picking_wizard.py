...
class PartialPickingWizard(models.TransientModel):
    _name = "nitrokey.partial.picking"
    _description = _("Nitrokey Partial Picking")
    ...
    def action_import(self):
        ...
        line = self.order_id.order_line.filtered(lambda l: l.product_id.type == 'product')
        if len(line) != 1:
            raise UserError("Can't use an order with more than 1 storable product")

        product = line.product_id
        parent = self.order_id.partner_id
        picking_type = self.env["stock.picking.type"].search([("barcode", "=", "WH-DELIVERY")])

        src_loc = picking_type.default_location_src_id or self.env.ref("stock.stock_location_customers")
        ...
        for row in self._read_csv():
            ...
            if not country:
                raise UserError(
                    "Invalid country %s. Please check the Odoo names or use the "
                    "ISO country code" % country_name
                )
