    ...
    def new_next_screen(self):
        ...
        if self.line_ids:
            ...
            values.update({
                ...
                'dst_partner_id': current_partner_ids[0], #self._get_ordered_partner(current_partner_ids)[-1].id,
                'state': 'selection',
            })
            self.write(values)
...
#     def _compute_hide_name_icon(self):
#         for record in self:
#             record.hide_name_icon = record.name==record.name2
    ...
    def action_skip(self):
        '''if self.partner_wizard_id.current_line_id:
            ...
            raise osv.except_osv(_('Error'), _("No dublicates found"))
        return self.partner_wizard_id.new_next_screen()'''
        ...
        if self.partner_wizard_id.current_line_id:
            ...
            raise Warning(_("No dublicates found")) #osv.except_osv(_('Error'), _("No dublicates found"))
    ...
    def _merge(self, cr, uid, ids,partner_ids, dst_partner=None, context=None):
        ...
        if len(partner_ids) > 3:
            raise Warning(_("For safety reasons, you cannot merge more than 3 contacts together. You can re-open the wizard several times if needed."))
        ...
        if set(partner_ids).intersection(child_ids):
            raise Warning(_("You cannot merge a contact with one of his parent."))

        if SUPERUSER_ID != uid and len(set(partner.email for partner in proxy.browse(cr, uid, partner_ids, context=context))) > 1:
            raise Warning(_("All contacts must have the same email. Only the Administrator can merge contacts with different emails."))
        ...
        if SUPERUSER_ID != uid and self._model_is_installed(cr, uid, 'account.move.line', context=context) and \
                self.pool.get('account.move.line').search(cr, SUPERUSER_ID, [('partner_id', 'in', [partner.id for partner in src_partners])], context=context):
            raise Warning(_("Only the destination contact may be linked to existing Journal Items. Please ask the Administrator if you need to merge several contacts linked to existing Journal Items."))

        call_it = lambda function: function(cr, uid, src_partners, dst_partner,context=context)
    ...
    def _update_foreign_keys(self, cr, uid, src_partners, dst_partner, context=None):
        res = self.pool['base.partner.merge.automatic.wizard']._update_foreign_keys(cr, uid, src_partners, dst_partner, context=context)
        return res
    
    def _update_reference_fields(self, cr, uid, src_partners, dst_partner, context=None):
        res = self.pool['base.partner.merge.automatic.wizard']._update_reference_fields(cr, uid, src_partners, dst_partner, context=context)
        return res
    ...
    def _update_values(self, cr, uid, src_partners, dst_partner, context=None):
        ...
        values = dict()
        #Skip merging of value for below fields because that are editing by user in the popup.
        form_fields = ['name','email', 'phone', 'street','street2', 'zip','city','state_id','country_id','is_company','newsletter_sendy','vat']
        for column, field in columns.iteritems():
            if field._type not in ('many2many', 'one2many') and not isinstance(field, old_fields.function) and column not in form_fields:
...
#         custom_partner_ids = set(map(int, this.custom_partner_ids))
        ...
        self._merge(cr, uid, ids,partner_ids, this.dst_partner_id, context=context)
        ...
        return this.partner_wizard_id.new_next_screen()
