...
class InvoiceExcelExport(models.TransientModel):
    ...
    def export_report_xlsx(self):
        ...
        header_style = easyxf('font:height 200;pattern: pattern solid, '
                              'fore_color blue; align: horiz center;font: '
                              'color white; font:bold True;' "borders: "
        ...
        for inv in self.invoice_ids:
            ...
            for order in inv.sale_order_ids:
                ...
                order_date = datetime.strptime(date_order,
                                               DEFAULT_SERVER_DATETIME_FORMAT).strftime('%d.%m.%Y')
                ...
                for pick in order.picking_ids:
                    ...
                    pick_date = datetime.strptime(date_pick,
                    ...
                    #        worksheet.write(
                    #        i, 8, delivery.carrier_tracking_ref or '', text_left)

                    # url = carrier and carrier.tracking_url_base or ''
                    # date_delivery = delivery.date.strftime(
                    # DEFAULT_SERVER_DATETIME_FORMAT)
                    # d = datetime.strptime(date_delivery,
                    # DEFAULT_SERVER_DATETIME_FORMAT)
                    # url = url.replace('<TAG>', str(d.day)).replace('<MONAT>',
                    # str(d.month)).replace('<JAHR>',
                    # str(d.year)) url = url.replace('<TRACKING-NUMMER>',
                    # delivery.carrier_tracking_ref)
        ...
        action = {
            ...
            'url': "/web/content/?model=invoice.excel.export&id=" + str(
                self.id) + "&field=file_data&download=true&filename=invoice_excel.xls",
            'target': 'self',
        }
        return action
