...
class TestWebsiteLangRedirect(HttpCase):
    ...
    def test_01_change_lang_redirect(self):
        ...
        resp = self.url_open("/website/lang/de_DE?r=/test/contactus", allow_redirects=False)
        self.assertIn(resp.status_code, (303, 200))
        redirect_url = resp.headers.get("Location")
        _logger.info("[DE] Status: %s | Redirected to: %s", resp.status_code, redirect_url)

    def test_02_default_language_redirect(self):
        """Test /website/lang redirection works cleanly."""
        resp = self.url_open("/website/lang/en_US?r=/test/contactus", allow_redirects=False)
        self.assertIn(resp.status_code, (303, 200))
        redirect_url = resp.headers.get("Location")
        _logger.info("[EN] Status: %s | Redirected to: %s", resp.status_code, redirect_url)
