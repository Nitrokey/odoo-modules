...
class ConsentController(Controller):
    ...
    def consent(self, access_token, **kwargs):
        try:
            ...
            if not mailing_list_contact:
                _logger.warning("No mailing subscription found for access token: %s", access_token)
            ...
            if not template:
                _logger.warning("No consent mail template found for subscription ID: %s", mailing_list_contact.id)
                return self.consent_failure()
