...
class ExportInvoicePdfZip(models.TransientModel):
    ...
    def action_export_pdf_zip(self):
        domain = [('invoice_date', '>=', self.from_date), ('invoice_date', '<=', self.to_date)]
        ...
        for invoice in invoices:
            ...
            report_name = safe_eval(report.print_report_name, {'object': invoice, 'time': datetime.now().strftime('%Y%m%d%H%S')})
        ...
        try:
            form_id = self.env['ir.model.data'].check_object_reference('account_invoice_pdf_export','view_export_invoice_pdf_zip_form_wizard')[1]
        except ValueError:
            form_id = False
