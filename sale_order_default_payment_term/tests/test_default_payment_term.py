...
class TestDefaultPaymentTerm(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        # Create test data
        self.pay_term_today = self.env["account.payment.term"].create(
            {
        ...
        self.pay_term_net_30_days = self.env["account.payment.term"].create(
            {
                "name": "Net 30 days",
                "line_ids": [
                    (
    ...
    def test_manual_payment_term_selection(self):
        ...
        sale_order = self.env["sale.order"].create(
            {
                "partner_id": self.partner.id,
                "payment_term_id": self.pay_term_net_30_days.id,
            }
        )
    ...
    def test_partner_payment_term(self):
        ...
        self.partner.property_payment_term_id = self.pay_term_net_30_days

        # Verify the partner's payment term is set correctly
        self.assertEqual(
            self.partner.property_payment_term_id.id,
            self.pay_term_net_30_days.id,
        ...
        _logger.info("Test payment term 2 ID: %s", self.pay_term_net_30_days.id)
        ...
        self.assertEqual(
            sale_order.payment_term_id.id,
            self.pay_term_net_30_days.id,
            "The payment term should be the one from the partner's "
            "property_payment_term_id",
        )

        # Clean up: Unset the payment term on the partner to avoid
