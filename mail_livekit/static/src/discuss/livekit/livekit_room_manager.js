...
export class LivekitRoomManager {
    ...
    setupRoomEventHandlers(room, channel, LivekitClient) {
        ...
        room.on(
            ...
            (track, publication, participant) => {
                ...
                if (track.kind === "video") {
                    ...
                    try {
                        const adapter =
                            this.env.services["discuss.livekit_rtc_adapter"];
                        const mediaStreamTrack = track?.mediaStreamTrack;
                        const type = this.publicationToType(publication);
                        if (adapter && channel?.id) {
                            adapter.setLivekitVideoTrackForIdentity(
                                channel.id,
                    ...
                    this.attachRemoteAudio(key, track, {
                        channel,
                        identity: participant.identity,
                    });
                }
            ...
            (track, publication, participant) => {
                ...
                if (track.kind === "video") {
                    ...
                    try {
                        ...
                        const type = this.publicationToType(publication);
                        if (adapter && channel?.id) {
                            adapter.removeLivekitVideoTrackForIdentity(
                                channel.id,
                                participant.identity,
                                {
                                    type,
                            ...
                            adapter.removeStreamForIdentity(
                                channel.id,
                                participant.identity,
                                {
                                    type,
                    ...
                    this.detachRemoteAudio(key, track, {
                        channel,
                        identity: participant.identity,
                    });
                }
        ...
        room.on(LivekitClient.RoomEvent.Disconnected, () => {
            ...
            const closePromise =
                channelToClose && rtc?.leaveCall
                    ? rtc.leaveCall(channelToClose)
                    : this.leave();
