...
class ResCompany(models.Model):
    ...
    use_dhl_parcel_de_shipping_provider = fields.Boolean(copy=False,
        string="Use DHL Parcel DE Shipping Provider",
        help="If use DHL Parcel DE shipping provider than value set TRUE.",
        default=False
    )
    dhl_parcel_de_api_url = fields.Char(string="DHL API URL", copy=False, default="https://api-sandbox.dhl.com")
    dhl_userid = fields.Char("DHL UserId", copy=False,
    ...
    dhl_password = fields.Char("DHL Password", copy=False,
    ...
    dhl_api_key = fields.Char("DHL API Key", copy=False,
                              help="Obtained via Get Access! (app creation) and manually approved by DHL.")
    dhl_tracking_url = fields.Char("DHL Tracking URL", copy=False,
