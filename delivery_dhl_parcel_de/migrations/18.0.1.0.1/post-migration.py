...
def migrate(cr, version):
    ...
    cr.execute("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'temp_dhl_company_data')")
    ...
    cr.execute("SELECT column_name FROM information_schema.columns WHERE table_name = 'carrier_account'")
    carrier_fields = {row[0] for row in cr.fetchall()}

    field_mapping = ['use_dhl_parcel_de_shipping_provider', 'dhl_parcel_de_api_url', 'dhl_userid', 
                    'dhl_password', 'dhl_api_key', 'dhl_api_secret', 'dhl_tracking_url', 'dhl_access_token']
    ...
    for data in companies_data:
        try:
            ...
            account_vals = {
                ...
                **{k: v for k, v in data.items() if v and k in carrier_fields and k in field_mapping}
            }
            
            if existing:
                update_vals = {k: v for k, v in account_vals.items() if k not in ['company_id', 'carrier_id']}
                if update_vals:
            ...
            failed += 1
            _logger.error(f"Error processing company {data.get('company_id', 'unknown')}: {e}")
    
    _logger.info(f"Migration completed: {success} successful, {failed} failed")
    
    # Archive temp table
    cr.execute("ALTER TABLE IF EXISTS temp_dhl_company_data RENAME TO temp_dhl_company_data_archived")
